...
const isReactComponentCandidate = (candidate: any): candidate is DemoComponentType => typeof candidate === 'function'

const getComponentNameAndValidate = (component: any, suggestedName: string): { name: string; isValid: boolean } => {
	const name = (component as any).displayName || (component as any).name || suggestedName
	const isValidName = /^[A-Z]/.test(name) || name.toLowerCase().includes('demo')
...
function App() {
	...
	const [selectedDemo, setSelectedDemo] = useState<DemoEntry | undefined>(undefined)
	...
	const addDemo = (component: any, baseName: string) => {
		...
		const { name: componentName, isValid } = getComponentNameAndValidate(component, baseName)
