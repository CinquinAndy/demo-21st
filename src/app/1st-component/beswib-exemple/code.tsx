...
export interface AnimatedHeroSectionProps {
	/** Main title with optional HTML (use <span> for special styling) */
	title: string
	/** Subtitle/description text (supports HTML with <br />) */
	subtitle: string
	/** Array of CTA buttons */
	buttons: HeroButton[]
	/** Optional background image path */
	backgroundImage?: string
	/** Optional side image path */
	sideImage?: string
	/** Optional background pattern (SVG path) */
	backgroundPattern?: string
	/** Theme - defaults to 'light', can also be 'dark' or controlled externally */
	theme?: 'light' | 'dark'
	/** Enable animated runners and cyclists (default: true) */
	enableAnimation?: boolean
	/** Global speed multiplier for animations (default: 0.75) */
	animationSpeed?: number
	/** Count multiplier for animated entities (default: 0.2) */
	animationDensity?: number
}

// ============================================================================
// ANIMATED LIFE COMPONENT
// ============================================================================
...
function AnimatedLife({ theme = 'light', globalSpeed = 0.75, countMultiplier = 0.2 }: AnimatedLifeProps) {
	const [entities, setEntities] = useState<RaceEntity[]>([])
	const nextWaveAtRef = useRef<number | null>(null)

	const [speedFactor, setSpeedFactor] = useState<number>(globalSpeed)
	const [countFactor, setCountFactor] = useState<number>(countMultiplier)
	...
	useEffect(() => {
		const compute = () => {
			...
			if (w <= 640) {
				setSpeedFactor(Math.max(0.1, globalSpeed + 0.3))
				setCountFactor(Math.max(0, countMultiplier * 0.5))
			} else {
				setSpeedFactor(globalSpeed)
				setCountFactor(countMultiplier)
			}
		}
		compute()
		window.addEventListener('resize', compute)
		return () => window.removeEventListener('resize', compute)
	}, [globalSpeed, countMultiplier])
	...
	return (
		<div className="absolute bottom-0 left-0 z-20 flex h-[45vh] w-screen md:h-[35vh]">
...
export function AnimatedHeroSection({
	...
	return (
		<div className="relative pb-0 md:pb-0 xl:pb-24">
			...
			{/* Animated Life */}
			{enableAnimation && (
				<AnimatedLife theme={theme} globalSpeed={animationSpeed} countMultiplier={animationDensity} />
			)}
							...
							<Link
								...
								className={
									button.variant === 'primary'
										? 'bg-primary text-primary-foreground ring-offset-background hover:bg-primary/90 focus-visible:ring-ring inline-flex h-11 items-center justify-center rounded-md px-3 text-sm font-medium shadow-sm transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none md:px-8 dark:shadow-none'
